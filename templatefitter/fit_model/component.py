...
class Component:
    def __init__(
        ...
        self._templates = None  # type: Optional[Tuple[Template, ...]]
        self._binning = None  # type: Optional[Binning]
        self._params = params  # type: ParameterHandler

        self._shared_yield = None  # type: Optional[bool]
        self._has_fractions = None  # type: Optional[bool]
        self._required_fraction_parameters = None  # type: Optional[int]

        self._initialize_templates(templates=templates)
        self._initialize_fractions(shared_yield=shared_yield)

        self._fraction_parameters = None  # type: Union[None, List[TemplateParameter], TemplateParameter]
    ...
    def _initialize_templates(
        ...
        assert self._templates is None
        assert self._binning is None
        ...
        if isinstance(templates, Template):
            ...
            if not all(isinstance(t, Template) for t in templates):
                raise ValueError(
                    "The parameter 'template' must be a Template or a List of Templates!\n"
            ...
            if not all(
                (
                    t.data_column_names == templates[0].data_column_names
                    if t.data_column_names is not None
                    else t.data_column_names is templates[0].data_column_names
                )
                for t in templates
            ):
                raise RuntimeError(
                    "The data_column_names of the templates you are trying to combine in this component"
            ...
            raise ValueError(
                f"The parameter 'template' must be a Template or a List of Templates!\n"
        ...
        if not all(t.params is self._params for t in self.templates):
            raise RuntimeError("The used ParameterHandler instances are not the same!")

    def _initialize_fractions(
        self,
        shared_yield: Optional[bool],
    ) -> None:
        assert self._shared_yield is None
        assert self._has_fractions is None
        assert self._required_fraction_parameters is None
        if self.number_of_subcomponents == 1:
            ...
            raise ValueError(
                "Please specify whether yields are shared via the argument shared_yields "
    ...
    def initialize_parameters(
        ...
        if self.required_fraction_parameters > 0:
            if fraction_parameters is None or (
                isinstance(fraction_parameters, List) and len(fraction_parameters) != self.required_fraction_parameters
            ):
                raise ValueError(
                    f"The component {self.name}, consisting of {self.number_of_subcomponents} templates,"
                    f"requires {self.required_fraction_parameters} ModelParameters of parameter_type "
                    f"'fraction' to be set, but you provided " + "'None'!"
                    if fraction_parameters is None
                    else f"a list containing {len(fraction_parameters)} parameters!"
                )
            ...
            if not all(isinstance(fraction_parameter, TemplateParameter) for fraction_parameter in fraction_parameters):
                raise ValueError(
                    f"Argument 'fraction_parameters' must be a list of TemplateParameters!\n"
            ...
            if not all(fraction_parameter.parameter_type == "fraction" for fraction_parameter in fraction_parameters):
                raise ValueError(
                    f"Expected TemplateParameters of parameter_type 'fraction', "
            ...
            for template, fraction_parameter in zip(self.templates[:-1], fraction_parameters):
            ...
            self.templates[-1].other_fraction_parameters = fraction_parameters

        else:
            if not (
                fraction_parameters is None or (isinstance(fraction_parameters, List) and len(fraction_parameters) == 0)
            ):
                raise ValueError("The component does not require fraction TemplateParameters, but you provided some!")
    ...
    def shared_yield(self) -> bool:
        assert self._shared_yield is not None
        return self._shared_yield

    @property
    def has_fractions(self) -> bool:
        assert self._has_fractions is not None
    ...
    def component_serial_number(self, component_serial_number: int) -> None:
        if self._component_serial_number is not None:
            raise RuntimeError(
                f"Trying to reset component serial number "
    ...
    def template_serial_numbers(self) -> Tuple[int, ...]:
        return tuple(template.serial_number for template in self.templates)
    ...
    def required_fraction_parameters(self) -> int:
        assert self._required_fraction_parameters is not None
    ...
    def data_column_names(self) -> Optional[List[str]]:
        if not all(
            t.data_column_names == self.templates[0].data_column_names
            if t.data_column_names is not None
            else t.data_column_names is self.templates[0].data_column_names
            for t in self.templates
        ):
            raise RuntimeError(
                f"Inconsistency in data_column_names of component {self.name}:\n\t-"
                + "\n\t-".join([f"{t.name}: {t.data_column_names}" for t in self.templates])
            )
        return self.templates[0].data_column_names
    ...
    def check_template_yield_indices(self):
        first_model_param = self.templates[0].yield_parameter.base_model_parameter
        if not all(first_model_param is t.yield_parameter.base_model_parameter for t in self.templates):
            raise RuntimeError(
                "Trying to setup a component from multiple templates which should share their "
                "yield parameter for templates which do not have the same yield parameter!\n"
                "The used templates use the following yield model parameters:\n"
                + "\n".join([f"{t.name}\n{t.yield_parameter.base_model_parameter.as_string()}" for t in self.templates])
            )
