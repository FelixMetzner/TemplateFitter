...
def gen_bkg(size, voi, voi_limits):
    df = pd.DataFrame(
        {
...
# Defining a HistVariable object for the fit to use. Note: The scope / fit range used here can be (and is) smaller then the one used for generating the dataset.
# This does not really make sense here but is sometimes beneficial. The fitter will later correct the yields so they apply to the entire dataset.
...
# Now that we're done creating the model, we can see how data and MC agree before the fit. Agreement should be perfect
...
pre_fit_plot_paths = fit_result_plotter_m2.plot_fit_result(
    use_initial_values=True, output_dir_path=output_folder, output_name_tag=""
)
...
for yield_parameter in param_handler.get_parameter_names_for_type(ParameterHandler.yield_parameter_type):
    significance_dict[yield_parameter] = fitter.get_significance(
        yield_parameter=yield_parameter, fix_nui_params=False, verbose=False
    )

print(
    f"The significance of the signal + background hypothesis vs. only background is {significance_dict['yield_signal']:.1f} sigma."
)
