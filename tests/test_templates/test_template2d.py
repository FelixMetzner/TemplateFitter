...
class TestHist2d(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.y = np.array(
            [3.5, 3.0, 3.2, 3.1, 3.6, 3.9, 3.4, 3.4, 2.9, 3.1, 3.7, 3.4, 3.0,
             ...
             3.2, 3.3, 3.0, 2.5, 3.0, 3.4, 3.]
        )

        self.bins = (4, 4)
        self.num_bins = self.bins[0] * self.bins[1]
        self.range = ((4, 6), (2, 4))
        self.hist = Hist2d(bins=self.bins, hist_range=self.range, data=(self.x, self.y))
    ...
    def test_add_variation(self):
        ...
        hup, _, _ = np.histogram2d(self.x, self.y, bins=self.bins, range=self.range, weights=up)
        hdown, _, _ = np.histogram2d(self.x, self.y, bins=self.bins, range=self.range, weights=down)
